🛑 RÈGLE CRITIQUE ET PRIORITAIRE (À LIRE ABSOLUMENT)
Avant CHAQUE tâche, tu DOIS lire ce fichier `ASSIST.md`.
Tout fichier Markdown (`.md`) de planification, d'audit, de documentation technique interne ou de réflexion (ex: `PLAN.md`, `BACK.md`, `AI.md`, etc.) DOIT être ignoré par Git.
👉 Sauf le `README.md`, TOUS les fichiers `.md` doivent être ajoutés au `.gitignore` s'ils n'y sont pas déjà.
👉 Si tu crées un nouveau fichier `.md` de ce type, ajoute-le IMMÉDIATEMENT au `.gitignore`.
...
1. Analyse le code avant de proposer un changement et comprends le code et le gestionnaire de package utilise.
